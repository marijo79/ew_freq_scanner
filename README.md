...
**Running with fewer channels than a previous run**: the metadata topic's partition count only ever grows (a Kafka topic can't shrink), so if you've run with, say, 3 HackRF ranges before and now run with just 1, the higher partitions still hold their old metadata from that earlier run — nothing deletes it. The viewer handles this correctly: every metadata message carries a `run_epoch` (the producer's own startup timestamp, identical across every channel published in one run), and the viewer only keeps the messages sharing the *highest* `run_epoch` it sees — that's this run's channels, and any stale leftovers from a larger previous run are ignored rather than shown as extra channels with wrong labels.
